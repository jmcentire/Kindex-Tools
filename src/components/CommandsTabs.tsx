...
const mcpTools = [
  { name: "search", desc: "Find relevant knowledge by query (--tags, --trusted-only)" },
  { name: "add", desc: "Add knowledge with type, weight, and tags" },
  { name: "edit", desc: "Correct or extend an existing node under edit policy" },
  { name: "supersede", desc: "Replace an additive node while preserving history" },
  { name: "context", desc: "Get tiered context, optionally trusted-only" },
  ...
  { name: "candidate_list", desc: "List quarantined automatic-capture candidates" },
  { name: "candidate_show", desc: "Inspect exact untrusted payload and freshness token" },
  { name: "candidate_accept", desc: "Promote a fresh candidate after explicit review" },
  { name: "candidate_reject", desc: "Reject a candidate with an audit reason" },
  { name: "candidate_prune", desc: "Expire candidates whose retention window elapsed" },
  { name: "candidate_erase", desc: "Erase a candidate and its minimal receipt" },
  { name: "verify", desc: "Assert verification provenance and valid-time" },
  { name: "invalidate", desc: "Record asserted invalidation and exclusive end time" },
  ...
  { name: "dream", desc: "Run consolidation: dedup, suggestions, and edge strengthening" },
  ...
  { name: "tag_resume", desc: "Resume with bounded, trusted state admission" },
  ...
  { name: "task_claim", desc: "Claim a task for an agent with an expiry" },
  { name: "task_release", desc: "Release or override a task claim" },
  { name: "coord_start", desc: "Start a short-lived agent coordination channel" },
  { name: "coord_join", desc: "Join a coordination channel and get a read cursor" },
  { name: "coord_post", desc: "Post a coordination message" },
  { name: "coord_read", desc: "Read coordination messages since a cursor" },
  { name: "coord_attach", desc: "Attach a graph node as a shared coordination resource" },
  { name: "coord_inject", desc: "Set, clear, or list standing member messages" },
  { name: "coord_list", desc: "List active or ended coordination channels" },
  { name: "coord_end", desc: "End a coordination channel with an optional summary" },
  { name: "lock_acquire", desc: "Acquire an advisory lock on a node" },
  { name: "lock_release", desc: "Release an advisory lock on a node" },
  { name: "remind_create", desc: "Create reminders with actions or Codex/OpenCode wakeups" },
...
const cliGroups = [
  {
    ...
    commands: [
      ...
      { name: "search", desc: "Search the graph (--tags, --mine, --trusted-only)" },
      ...
      { name: "candidate", desc: "Review quarantined automatic captures" },
      { name: "verify", desc: "Assert verification and valid-time" },
      { name: "invalidate", desc: "Record explicit invalidation" },
    ],
  },
  {
    title: "Graph",
    commands: [
      ...
      { name: "context", desc: "Get tiered or trusted-only context output" },
    ],
  },
  {
    title: "Operations",
  ...
  },
  {
    ...
    commands: [
      ...
      { name: "remind", desc: "Time-based triggers with actions and wakeups" },
